...
// ── Scoped project keys — advanced, collapsed by default ──────────────────────
// Most users never open this. It's strictly additive: creating or revoking a
// project key never touches the default vault key shown above it.
function ScopedKeys({ apis }) {
  ...
  const activeApis = (apis || []).filter(a => a.state === 'active')
  const authHeader = { Authorization: `Bearer ${localStorage.getItem('token')}` }

  async function loadKeys() {
    setLoading(true)
    try {
      const res = await fetch(`${BASE}/user/keys`, { headers: authHeader })
      const d = await res.json()
      setKeys(d.scoped_keys || [])
    } catch {}
    setLoading(false)
  }

  useEffect(() => { if (expanded) loadKeys() }, [expanded])
  ...
  async function createKey() {
    ...
    try {
      const res = await fetch(`${BASE}/user/keys`, {
        ...
        body: JSON.stringify({ label: newLabel || 'Untitled key', scopes: fullAccess ? null : newScopes }),
      })
      const d = await res.json()
      setRevealedKey({ key: d.key, label: d.label })
      setNewLabel(''); setNewScopes([]); setFullAccess(true)
      loadKeys()
    } catch {}
    setCreating(false)
  }
  ...
  async function revoke(id) {
    ...
    try {
      await fetch(`${BASE}/user/keys/${id}`, { method: 'DELETE', headers: authHeader })
      loadKeys()
    } catch {}
  }

  function toggleScope(slug) {
    setNewScopes(p => p.includes(slug) ? p.filter(s => s !== slug) : [...p, slug])
  }
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-5 mb-4">
      <button onClick={() => setExpanded(e => !e)} className="w-full flex items-center justify-between text-left">
        <div>
          <div className="text-sm font-bold text-gray-900">Advanced: project keys</div>
          <div className="text-xs text-gray-400 mt-0.5">Create a separate key scoped to specific APIs — keeps projects isolated from each other</div>
        </div>
        <span className="text-gray-300 text-sm flex-shrink-0 ml-3">{expanded ? '▾' : '▸'}</span>
      </button>
        ...
        <div className="mt-4 pt-4 border-t border-gray-50 space-y-4">
          {revealedKey && (
            <div className="p-3 bg-green-50 border border-green-100 rounded-lg">
              <div className="text-xs font-semibold text-green-800 mb-1.5">✓ "{revealedKey.label}" created — copy this now, it won't be shown again</div>
              <div className="flex items-center gap-2">
                <code className="flex-1 text-xs bg-white border border-green-200 rounded px-2 py-1.5 truncate">{revealedKey.key}</code>
                <button onClick={() => navigator.clipboard?.writeText(revealedKey.key)}
                  className="text-xs text-green-700 font-medium px-2 flex-shrink-0">Copy</button>
              </div>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="text-xs text-gray-300">No project keys yet.</div>
          ) : (
            <div className="space-y-2">
              {keys.map(k => (
                <div key={k.id} className="flex items-center justify-between gap-3 p-2.5 bg-gray-50 rounded-lg">
                  <div className="min-w-0">
                    <div className="text-xs font-semibold text-gray-800 truncate">{k.label}</div>
                    <div className="text-[10px] text-gray-400">
                      {k.scopes ? k.scopes.join(', ') : 'Full access'} · Created {new Date(k.created_at).toLocaleDateString()}
                    </div>
                  </div>
                  <button onClick={() => revoke(k.id)} className="text-xs text-red-500 hover:text-red-700 font-medium flex-shrink-0">Revoke</button>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="pt-3 border-t border-gray-50 space-y-2.5">
            <input value={newLabel} onChange={e => setNewLabel(e.target.value)}
              placeholder="Key name — e.g. 'IVey production'"
            ...
            <label className="flex items-center gap-2 text-xs text-gray-600">
              ...
              Full access (all APIs)
            </label>
            {!fullAccess && (
              <div className="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto p-2 bg-gray-50 rounded-lg">
                {activeApis.map(a => (
                  <button key={a.slug} type="button" onClick={() => toggleScope(a.slug)}
                    className={`px-2 py-1 text-[10px] rounded-md border transition-colors ${
                      newScopes.includes(a.slug) ? 'bg-gray-900 text-white border-gray-900' : 'bg-white text-gray-500 border-gray-200 hover:border-gray-400'
                    }`}>
                    {a.name}
                  </button>
                ))}
            ...
            <button onClick={createKey} disabled={creating || (!fullAccess && newScopes.length === 0)}
              className="w-full py-2 bg-gray-900 text-white text-xs rounded-lg font-semibold hover:bg-gray-800 disabled:opacity-40 transition-colors">
  ...
  return (
    <div className="min-h-screen bg-[#fafafa]">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        {tab==='billing'     && <Billing me={me} setMe={setMe} vaultKey={vaultKey} setVaultKey={setVaultKey} apis={apis}/>}
